...
import { listCachedPagesProjects } from '../../../../server/workersPages';
...
export const GET: APIRoute = async ({ locals, request }) => {
  try {
    ...
    const result = await listCachedPagesProjects(db, userEmail, { page, pageSize, search });
    return Response.json({ projects: result.projects, total: result.total, page, pageSize });
  } catch {
    // 瞬时存储故障（如 dev 模式 D1 代理断连）返回规范 JSON，前端走错误横幅 + 重试
    return jsonError('Failed to query pages projects, please retry', 503);
  }
};
